...
{
	echo '<p>Onclick show popup.';
  	echo '<br><a href="http://www.gopiplus.com/work/2011/12/17/wordpress-plugin-onclick-show-popup-for-content/">';
	echo 'click here</a></p>';
}
...
add_filter('the_content','OnclickShowPopup_filter');

function OnclickShowPopup_filter($content)
{
	return 	preg_replace_callback('/\[ONCLICK-SHOW-POPUP:(.*?)\]/sim','OnclickShowPopup_filter_Callback',$content);
}

function OnclickShowPopup_filter_Callback($matches) 
{
	...
	$scode = $matches[1];
	list($group_cap, $group) = split('[=.-]', $scode);
...
add_action('init', 'OnclickShowPopup_add_javascript_files');
